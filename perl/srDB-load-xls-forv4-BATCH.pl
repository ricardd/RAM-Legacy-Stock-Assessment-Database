...
# Last modified: Time-stamp: <2011-06-24 12:17:40 (srdbadmin)>
# Daniel Ricard
...
# 2008-08-25: modifications to perform INSERT statements as a transaction
...
$assessyear = ($oWkS->{Cells}[$oWkS->{MinRow}+6][2]->Value) . "-" . ($oWkS->{Cells}[$oWkS->{MaxRow}][2]->Value) ;
# print ("$assessyear\n");
#$oWkS->{MaxRow} \n"
...
#for($iR = $oWkS->{MinRow}+5; defined $oWkS->{MaxRow} && $iR <= $oWkS->{MaxRow}-1; $iR++)
...
for($iC = $oWkS->{MinCol}+2;
      ...
      $iC++)
  {
    ...
    for($iR = $oWkS->{MinRow}+6;
      defined $oWkS->{MaxRow} && $iR <= $oWkS->{MaxRow} ;
      $iR++)

      {
	#print("$iC $iR \n");
	$tsmetric = $oWkS->{Cells}[4][$iC]->Value;
	$tsunit = $oWkS->{Cells}[5][$iC]->Value;

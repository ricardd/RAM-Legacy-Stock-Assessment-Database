-- remove stocks from the tables "srdb.spfits_Schaefer_Kbound2maxTB" and "srdb.spfits_Schaefer_Kbound5maxTB" for those assessid deemed appropriate 
-- from earlier work in schaefer-inclusion.sql
-- Last modified Time-stamp: <2011-06-16 15:09:26 (srdbadmin)>
...
CREATE TABLE srdb.spfits_schaefer_Kbound2maxTB AS (
SELECT * FROM srdb.spfits_schaefer_Kbound2maxTB_all
WHERE assessid in (
...
COMMENT ON TABLE srdb.spfits_schaefer_Kbound2maxTB IS 'This table stores the parameter estimates from the Schaefer surplus production model ran against the catch and total biomass timeseries. The assessment included in this table are only those who have been manually checked for proper fit.';


DROP TABLE srdb.spfits_schaefer_Kbound5maxTB;


CREATE TABLE srdb.spfits_schaefer_Kbound5maxTB AS (
SELECT * FROM srdb.spfits_schaefer_Kbound5maxTB_all
WHERE assessid in (
'AFSC-ALPLAICBSAI-1972-2008-MELNYCHUK',
'AFSC-ARFLOUNDBSAI-1970-2008-STANTON',
'AFSC-ARFLOUNDGA-1958-2010-STANTON',
...
COMMENT ON TABLE srdb.spfits_schaefer_Kbound5maxTB IS 'This table stores the parameter estimates from the Schaefer surplus production model ran against the catch and total biomass timeseries. The assessment included in this table are only those who have been manually checked for proper fit.';

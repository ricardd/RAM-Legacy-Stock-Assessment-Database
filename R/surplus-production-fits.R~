...
pdf("spfits-Schaefer-all.pdf", width=8, height=10, title="All Schaefer fits")
...
qu <- paste("select * from srdb.spfits_schaefer") ## bring back parameter estimates from srdb.spfits_schaefer
sp.fit.srdb <- sqlQuery(chan,qu)

pdf("spfits-Schaefer-MANUALLY-CLEANED.pdf", width=8, height=10, title="Schaefer fits manually cleaned")
par(mfrow=c(5,4),mar=c(1,1,1,1), oma=c(3.5,3,0,0))

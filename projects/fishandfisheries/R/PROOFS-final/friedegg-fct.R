...
srdb.stock s,
srdb.taxonomy t,
srdb.lmetostocks lts,
srdb.lmerefs lr,
(select assessid, min(tsyear) || '-' || max(tsyear) as timespan from srdb.timeseries group by assessid) as ts
WHERE
s.stockid=lts.stockid AND
lts.stocktolmerelation = 'primary' AND
lts.lme_number = lr.lme_number AND 
...
GROUP BY 
r.risentry, 
a.assessid, lr.lme_name, s.stocklong, t.scientificname, t.ordername, ts.timespan, 
m.country,
...
mtl.dat <- sqlQuery(chan, qu)
 ...
 crosshair.for.table <- data.frame(mgmt=crosshair.for.table.temp$mgmt, country=crosshair.for.table.temp$country, lme=crosshair.for.table.temp$lmename, stock=crosshair.for.table.temp$stocklong.y ,scientificname=crosshair.for.table.temp$scientificname.y, ordername=crosshair.for.table.temp$ordername, TL=ifelse(crosshair.for.table.temp$ratio.x,crosshair.for.table.temp$tl,""), assessmethod=crosshair.for.table.temp$category, timespan=crosshair.for.table.temp$timespan.y, currentyear=crosshair.for.table.temp$maxyr.x, Bratio=ifelse(crosshair.for.table.temp$btype=="yes",round(crosshair.for.table.temp$ratio.x,2), paste(round(crosshair.for.table.temp$ratio.x,2),"*")), Uratio=ifelse(crosshair.for.table.temp$utype=="yes",round(crosshair.for.table.temp$ratio.y,2), paste(round(crosshair.for.table.temp$ratio.y,2),"*")))
...
my.caption <- c("Summary of population-dynamics model based assessments in the RAM Legacy database, including the management body (acronyms from Table 1), assessment method, timespan of their longest time series data, estimated ratios of current biomass to the biomass at MSY and current harvest rate to the harvest rate that results in MSY. Estimated ratios were preferentially obtained directly from the assessment document or derived from surplus production models. When both SSBmsy and Bmsy reference points were available, SSB was chosen preferentially.")
  ...
  my.table.S2 <- xtable(crosshair.for.table, caption=my.caption, label=c("tab:crosshair"), digits=2, align="p{1.5cm}p{1.5cm}p{1.5cm}p{1.5cm}p{3cm}p{3cm}p{2.5cm}p{0.9cm}p{1.4cm}p{0.9cm}p{0.9cm}p{0.9cm}p{1cm}")

## Table S1
  print(my.table.S2, type="latex", file="../../tex/PROOFS-final/Table-S1.tex", include.rownames=FALSE, floating=FALSE, tabular.environment="longtable", caption.placement="bottom", sanitize.text.function=I)

## html table to use in Word

  my.table.S2 <- xtable(crosshair.for.table, caption=my.caption, label=c("tab:crosshair"), digits=2, align="p{1.8cm}p{1.3cm}p{1.8cm}p{3.5cm}p{3.5cm}p{2.5cm}p{0.9cm}p{1.0cm}cp{0.9cm}cp{0.9cm}c")
 print(my.table.S2, type="html", file="../../tex/PROOFS-final/Table-S1.html", include.rownames=FALSE, floating=FALSE, tabular.environment="longtable", caption.placement="bottom", sanitize.text.function=I)

...
# Time-stamp: <2010-03-08 15:14:39 (srdbadmin)>
...
#----------------------------------------------------
my $lhsql = qq{select * from (select bioid, biovalue, biounitsshort from srdb.bioparams b, srdb.biometrics c where assessid like \'$assessid\' and b.bioid=c.biounique) as a where a.bioid in ('REC-AGE-yr', 'SSB-AGE-yr', 'TB-AGE-yr', 'F-AGE-yr-yr', 'M-1/T', 'M-1/yr', 'NATMORT-1/yr', 'A50-yr', 'L50-cm')}; #, 'MORATOR-yr-yr')};
...
#---------------
# Blim
if(!$Blim eq "" && !$ssbcurr eq "" && eval { $Blim +=0 }){
my @ratio1=("\$SSB_{$ssbyrcurr}/B_{lim}\$",sprintf("%.3f", $ssbcurr/$Blim),"");

...
geo.classification <-  sqlQuery(chan,paste("select * from ",qu.geo))

all.classification <- merge(taxo.classification,geo.classification)
...
## actual plot
pdf("RSC-Hutchings-multiF-plot.pdf", height=11*1.6, width=11)
par(mfrow=c(3,1))
...
## Atlantic only
in.dat <- subset(f.msy.canada, geo=='NWAtl')
plot(0,0,xlim=xl, ylim=yl,xlab="",ylab="", axes=FALSE)
axis(side=1, at=seq(1880,2000,20),cex=0.8)
axis(side=2, at=c(log(0.01),log(0.1),log(0.5),log(1),log(5),log(10)), labels=c(0.01,0.1,0.5,1.0,5,10),cex=0.8)
mtext(expression(U[curr]/U[MSY]), side=2, line=1, outer=TRUE, cex=1)
...
plot.label <- "Atlantic"
  legend('bottomleft', paste(plot.label," (n=",aa,")",sep=""))
#legend(xl[1],log(3), "Demersal"), col=c("blue","red"),pch=c(19,19), lty=c(1,1))
...
# Pacific only
in.dat <- subset(f.msy.canada, geo=='NEPac')
plot(0,0,xlim=xl, ylim=yl,xlab="",ylab="", axes=FALSE)
axis(side=1, at=seq(1880,2000,20),cex=0.8)
axis(side=2, at=c(log(0.01),log(0.1),log(0.5),log(1),log(5),log(10)), labels=c(0.01,0.1,0.5,1.0,5,10),cex=0.8)
mtext(expression(U[curr]/U[MSY]), side=2, line=1, outer=TRUE, cex=1)

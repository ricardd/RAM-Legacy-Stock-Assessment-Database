...
## DR, CM
## date:
## Time-stamp: <2010-05-12 15:48:06 (srdbadmin)>
...
if(multipanel){
  pdf("CBD-Supp-fig1_v1.pdf", width=8, height=10, title = "CJFAS CBD Short Comm. Figure S1")
  par(mfrow=c(5,3),mar=c(1,2,1,1), oma=c(3.5,8,0,0), las=1)
...
qu.salt <- paste("select bmsy from srdb.spfits where assessid = '", a[i], "'"
            ,sep="")
salt.bmsy <- sqlQuery(chan, qu.salt)

qu.pepper <- paste("select distinct(biovalue) from srdb.tsrelative_explicit_view where bioid like 'SSBmsy%' and assessid = '", a[i], "'"
            ,sep="")
pepper.bmsy <- sqlQuery(chan, qu.pepper)


my.cut<-which(temp$tsyear==cutoff)
...
mcont <- lm(log(ssb) ~ yr + post:I(yr-my.cut), data=temp)
...
if(!multipanel){par(mfrow=c(2,1),mar=c(1,5,1,2), oma=c(2,2,4,3),lab=c(5,3,7), cex=1, las=1)}

lower<- range(log(temp$ssb))[1]

  if(dim(pepper.bmsy)[1]) {lower <- min(lower,unlist(log(pepper.bmsy)))}
...
if(multipanel){
  ...
  abline(v=cutoff, lty=1, lwd=0.5, col=gray(0.7))
  
  # add BRP here
  #if(dim(salt.bmsy)[1]) {abline(h=log(salt.bmsy), lty=2, lwd=0.8, col=gray(0.7))}
  if(dim(pepper.bmsy)[1]) {abline(h=log(pepper.bmsy), lty=1, lwd=0.8, col=gray(0.7))}
  
#  if(((i/20)-floor(i/20))==0){
    mtext(side=1, line=1.5, text="Year", cex=1.2, outer=TRUE)
    mtext(side=2, line=1.25, text="ln(SSB)", cex=1.2, outer=TRUE ,las=3)
##    mtext(side=2, line=1.75, text="(Metric tonnes)", cex=1.2, outer=TRUE)
#  }
}else{
...
write.csv(c(1,5),"all-timestamp.csv")

##q("yes")

## sandbox

#ccc<-which(temp$tsyear==cutoff)
...
#mcont <- lm(log(ssb) ~ yr + post:I(yr-ccc), data=temp)
#lines(temp$yr,predict(mcont))
#lines(temp$yr,pred, lty=2, lwd=2, col="purple")

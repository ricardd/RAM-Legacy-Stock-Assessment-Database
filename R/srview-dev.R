...
# Started: 2008-07-22
#
# Time-stamp: <2008-10-15 18:32:41 (mintoc)>
#
...
srview.fct("NIWA-HOKIENZ-1972-2007-FRANCIS", "SSB-MT", "R-E00", "ER-none")
srview.fct("NIWA-HOKIWNZ-1972-2007-FRANCIS", "SSB-MT", "R-E00", "ER-none")
dev.off()
...
# following the same layout as RAM's original report




srviewRAM.fct <- function(assess.id, ssb, r, f, c) {
qu <- paste("select st.scientificname, st.commonname, st.areaID, st.stocklong from srdb.assessment aa, srdb.stock st where aa.stockid=st.stockid and assessid = '", assess.id, "'", sep="")
details <- sqlQuery(chan, qu, errors= TRUE)
...
qu <- paste("
SELECT * from srdb.timeseries where assessid=",
"'",assess.id,"' AND tsid=","'",c,"'", sep="")
c.data <- sqlQuery(chan,qu)

data.back <- data.frame(ID=ssb.data$assessid, YR=ssb.data$tsyear, SSB=ssb.data$tsvalue, R=r.data$tsvalue, F=f.data$tsvalue, C=c.data$tsvalue)

# plot SSB and R over the time span of the data
par(mfrow=c(4,2), mar=c(5,4,3,4), cex=0.9)
plot(data.back$YR, data.back$R, xlab="", ylab="", type='l', lwd=2, lty=1)
mtext(side=2, r, line=2)
mtext(side=1, "Year", line=2)
par(new=TRUE)
plot(data.back$YR, data.back$SSB, type='l', lwd=2, lty=2, axes=FALSE, xlab="", ylab="")
axis(side=4)
mtext(side=4, ssb, line=2)

t <- subset(data.back, (!is.na(SSB)) & (!is.na(R)))
plot(t$SSB, t$R, type='l', xlab="",ylab="", log='xy')
mtext(side=2, r, line=2)
mtext(side=1, ssb, line=2)

y5.ssb <- t$SSB[t$YR %% 5 == 0]
y5.r <- t$R[t$YR %% 5 == 0]
y5.txt <- t$YR[t$YR %% 5 == 0]
...
plot(data.back$YR, data.back$F, type='l', lwd=2, lty=1, col='red', xlab="Year", ylab="F")
par(new=TRUE)
plot(data.back$YR, data.back$C, type='l', lwd=2, lty=1, col='black', axes=FALSE, xlab="", ylab="C")
axis(side=4)

# return(data.back)
} # end function srviewRAM.fct


srviewRAM.fct("AFWG-CODNEAR-1943-2006-MINTO", "SSB-1-MT", "R-1-E00", "F-1-1/T","TL-1-MT")

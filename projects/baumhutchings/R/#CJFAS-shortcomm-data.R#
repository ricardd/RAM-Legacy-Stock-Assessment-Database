...
qu.ts <- paste ("
(select aa.stockid, v.assessid, v.tsyear, v.ssb from srdb.timeseries_values_view v, srdb.assessment aa where aa.assessid=v.assessid AND v.assessid in (select aa.assessid from (select assessid, max(tsyear) - min(tsyear) as nyears, min(tsyear) as minyear, max(tsyear)-1992 as nyearspost1992 from srdb.timeseries_values_view where ssb is not null and assessid in (select assessid from srdb.assessment where recorder != \'MYERS\' and stockid in (select stockid from srdb.stock where tsn in (select tsn from srdb.taxonomy where classname in (\'Actinopterygii\',\'Chondrichthyes\')))) group by assessid) as aa where nyears>=25 and minyear<=1978 and nyearspost1992>=10) and ssb is not null) as ts
", sep="")
...
tt.taxocategory,
gg.geo,
ts.stockid, ss.stocklong, ts.assessid, ts.tsyear, ts.ssb
FROM", qu.taxo, ",", qu.geo, ",", qu.ts, ",srdb.stock ss ",
"WHERE
...
# print-friendly list of stocks by geographic region
oo <- order(dat$geo)
stocklist <- unique(data.frame(area=as.character(dat$geo[oo]),stockid=as.character(dat$stockid[oo]),stocklong=as.character(dat$stocklong[oo])))
write.table(file="stocklist-CJFAS-shortcomm.csv",stocklist,sep=",", row.names=FALSE)

...
-- Last modified Time-stamp: <2011-08-02 15:09:47 (srdbadmin)>
...
CREATE TABLE srdb.spfits_fox AS (
SELECT * FROM srdb.spfits_fox_all
WHERE assessid in (
...
'DFO-POLL4X5YZ-1980-2006-PREFONTAINE',
...
COMMENT ON TABLE srdb.spfits_fox IS 'This table stores the parameter estimates from the Fox surplus production model ran against the catch and total biomass timeseries. The assessment included in this table are only those who have been manually checked for proper fit.';


COMMENT ON TABLE srdb.spfits IS 'This table stores the parameter estimates from the Fox surplus production model ran against the catch and total biomass timeseries. The assessment included in this table are only those who have been manually checked for proper fit.';


CREATE TABLE srdb.spfits_fox_temp AS (
SELECT
a.assessid,
...
CREATE TABLE srdb.spfits_fox_summary AS (
SELECT
spt.assessid,
spt.catchandbiomass,
spt.convergence,
...
DROP TABLE srdb.spfits_fox_temp;

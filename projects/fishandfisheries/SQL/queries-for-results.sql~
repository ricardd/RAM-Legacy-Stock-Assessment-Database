...
where recorder !='MYERS' and assess=1
)
UNION
(
...
srdb.assessment aa
WHERE
aa.recorder !='MYERS' and aa.assess=0
)
UNION
...
aa.recorder !='MYERS' and aa.assess=1 AND
...
UNION
(
select cast('REF:SQL:NUMCATCHSERIES' as varchar(100)) as flag, cast(count(assessid) as varchar(100)) as value from (select distinct assessid from srdb.newtimeseries_values_view where catch_landings is not null and assessid in (select assessid from srdb.assessment where assess=1 and recorder !='MYERS')) as aa
)
UNION
(
select cast('REF:SQL:NUMSSBSERIES' as varchar(100)) as flag, cast(count(assessid) as varchar(100)) as value from (select distinct assessid from srdb.newtimeseries_values_view where ssb is not null and assessid in (select assessid from srdb.assessment where assess=1 and recorder !='MYERS')) as aa
)
UNION
(
select cast('REF:SQL:NUMRSERIES' as varchar(100)) as flag, cast(count(assessid) as varchar(100)) as value from (select distinct assessid from srdb.newtimeseries_values_view where r is not null and assessid in (select assessid from srdb.assessment where assess=1 and recorder !='MYERS')) as aa
)
UNION
(
select cast('REF:SQL:NUMASSESSBIOREF' as varchar(100)) as flag, cast(count(distinct b.assessid) as varchar(100)) as value from srdb.assessment a, srdb.bioparams b where b.bioid in (select biounique from srdb.biometrics where a.assessid = b.assessid AND biounique like '%B%' and subcategory = 'REFERENCE POINTS ETC.') and a.recorder != 'MYERS' and a.assess=1 
)
UNION
(
select cast('REF:SQL:PERCENTASSESSBIOREF' as varchar(100)) as flag, cast((num.numassessmentref*100)/(den.totnumassessments*1) as varchar(100)) as value from (select count(distinct b.assessid) as numassessmentref from srdb.assessment a, srdb.bioparams b where b.bioid in (select biounique from srdb.biometrics where a.assessid = b.assessid AND biounique like '%B%' and subcategory = 'REFERENCE POINTS ETC.') and a.recorder != 'MYERS') as num, (select count(*) as totnumassessments from srdb.assessment where recorder != 'MYERS' and assess=1 ) as den
